...
def replace_dashed_abbreviations(text: str, abbrev_map: dict) -> str:
    """Replaces dash-connected abbreviations in text."""
    ...
    for abbr in sorted(dashed_abbrevs.keys(), key=lambda x: -len(x)):
        ...
        text = pattern.sub(dashed_abbrevs[abbr], text)
    return text
...
def remove_space_around_numeric_dots(text: str) -> str:
    """Removes space around dots that appear between two digits."""
...
def add_space_between_number_and_word(text: str) -> str:
    """Adds a space between a number and an adjacent word (e.g., '7.5cm' -> '7.5 cm')."""
...
def process_abbreviations(text: str, abbrev_map: Dict[str, str]) -> str:
    ...
    step1_processed = replace_dashed_abbreviations(text, abbrev_map)
    step2_processed = normalize_text_spacing(step1_processed)
    # Step 3: Handle numeric dots
    step3_processed = remove_space_around_numeric_dots(step2_processed)
    # Step 4: Add space between numbers and words
    step4_processed = add_space_between_number_and_word(step3_processed)
    # Step 5: Replace standard abbreviations
    final_output = replace_standard_abbreviations(step4_processed, abbrev_map)

    # Final normalization of whitespace after all steps
